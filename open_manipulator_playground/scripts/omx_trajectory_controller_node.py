...
class OmxTrajectoryControllerNode(Node):
    ...
    def __init__(self):
        ...
        # Subscriptions
        self.traj_sub = self.create_subscription(PoseArray, self.get_parameter('trajectory_topic').value, self.trajectory_callback, 10)
        self.status_sub = self.create_subscription(Bool, '/drawing_status', self.status_callback, 10)
        self.trigger_sub = self.create_subscription(Bool, '/start_drawing', self.trigger_callback, 10)
        ...
        self.leader_traj_sub = self.create_subscription(JointTrajectory, '/leader/joint_trajectory', self.leader_traj_callback, 10)
        self.leader_gripper_sub = self.create_subscription(JointTrajectory, '/leader/gripper_controller/joint_trajectory', self.leader_gripper_callback, 10)
        ...
        # Internal State
        self.is_executing, self.points, self.queued_points = False, [], []
    ...
    def parameter_callback(self, params):
        for param in params:
            if param.name == 'drawing_z': self.z_offset = param.value
            elif param.name == 'hover_height': self.hover_height = param.value
            elif param.name == 'drawing_segment_dur': self.base_dur = param.value
            elif param.name == 'resample_step': self.resample_step = param.value
            elif param.name == 'home_x': self.home_x = param.value
            elif param.name == 'home_y': self.home_y = param.value
            elif param.name == 'home_z': self.home_z = param.value
            elif param.name == 'approach_duration': self.approach_dur = param.value
            elif param.name == 'home_duration': self.home_dur = param.value
            elif param.name == 'joint5_angle': self.joint5_angle = param.value
        ...
        if self.last_positions is not None:
            ...
            for i in range(min(len(current_pos), len(self.last_positions))):
                if abs(current_pos[i] - self.last_positions[i]) > 0.002:
                    is_moving = True
                    break
            if not is_moving: self.stable_frames += 1
            else: self.stable_frames = 0
    ...
    def trigger_callback(self, msg):
        if msg.data and self.queued_points: self.start_mission(self.queued_points); self.queued_points = []
    ...
    def get_quaternion_from_euler(self, r, p, y):
        cy, sy, cp, sp, cr, sr = math.cos(y*0.5), math.sin(y*0.5), math.cos(p*0.5), math.sin(p*0.5), math.cos(r*0.5), math.sin(r*0.5)
        return [cy*cp*cr + sy*sp*sr, cy*cp*sr - sy*sp*cr, sy*cp*sr + cy*sp*cr, sy*cp*cr - cy*sp*sr]

    def publish_movel(self, x, y, z, duration):
        msg = MoveL()
        msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z = float(x), float(y), max(0.001, float(z))
        pan, pitch, roll = math.atan2(y, x), math.radians(90), math.radians(-90)
        q = self.get_quaternion_from_euler(roll, pitch, pan)
        msg.pose.pose.orientation.w, msg.pose.pose.orientation.x, msg.pose.pose.orientation.y, msg.pose.pose.orientation.z = q[0], q[1], q[2], q[3]
    ...
    def resample_path(self, points_xy):
        if len(points_xy) < 2: return points_xy
        dists = [0.0]
        for i in range(1, len(points_xy)):
            dists.append(dists[-1] + math.sqrt((points_xy[i][0]-points_xy[i-1][0])**2 + (points_xy[i][1]-points_xy[i-1][1])**2))
        if dists[-1] < self.resample_step: return points_xy
    ...
    def trajectory_callback(self, msg):
        ...
        for pi, path in enumerate(paths):
            ...
            for p in path:
                ...
                if r < 0.08 or r > 0.32: scale = min(max(r, 0.08), 0.32)/r; cx*=scale; cy*=scale
                raw.append((cx, cy))
            ...
            for i in range(1, len(resampled)):
                ...
                if i == 1: dur = self.base_dur * 2.5
                
                if i < len(resampled)-1:
                    v1, v2 = (pt[0]-resampled[i-1][0], pt[1]-resampled[i-1][1]), (resampled[i+1][0]-pt[0], resampled[i+1][1]-pt[1])
                    n1, n2 = math.sqrt(v1[0]**2+v1[1]**2), math.sqrt(v2[0]**2+v2[1]**2)
                    if n1>0 and n2>0:
                        ang = math.acos(max(-1.0, min(1.0, (v1[0]*v2[0]+v1[1]*v2[1])/(n1*n2))))
                        if ang > math.radians(30): dur = 0.18
                        elif ang > math.radians(10): dur = 0.10
                        elif ang > math.radians(2): dur = 0.07
                r_d = math.sqrt(pt[0]**2 + pt[1]**2)
                if r_d > 0.21: dur *= 2.0
                elif r_d > 0.18: dur *= 1.5
                track.append({"x": pt[0], "y": pt[1], "z": self.z_offset, "dur": dur})
            
            lx, ly = resampled[-1]
            # Lift
            for p_lift in [{"x": lx, "y": ly, "z": self.z_offset, "dur": 0.5}, {"x": lx, "y": ly, "z": 0.08, "dur": 2.0}, {"x": lx, "y": ly, "z": 0.08, "dur": 1.0}]:
                track.append(p_lift)
        
        # Home Position
        for p_home in [{"x": self.home_x, "y": self.home_y, "z": self.home_z - 0.001, "dur": self.home_dur}, {"x": self.home_x, "y": self.home_y, "z": self.home_z, "dur": 3.0}]:
            track.append(p_home)
...
def main(args=None):
    rclpy.init(args=args); node = OmxTrajectoryControllerNode(); rclpy.spin(node); node.destroy_node(); rclpy.shutdown()
